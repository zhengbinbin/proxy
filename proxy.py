...
class Proxy():
    def __init__(self, pppoeLog, checkUsefullog, ipLog, port):
    ...
    def pppoe(self):
        ...
        rs = subprocess.call(self.pppoeStop, shell=True)
        if rs != 0:
            re = os.popen('ps -aux | grep pppoe | grep -v "grep" | wc -l').read()
            A = re.split()
            if A[0] != '0':
                subprocess.call("ps -aux | grep pppoe | grep -v 'grep' | awk '{print $2}' | xargs kill", shell=True)
                with open(self.pppoeLog, 'a') as f:
                    f.write(date + '  ' + '拨号程序故障，已杀死所有pppoe进程\n')
        time.sleep(3)
        ps = subprocess.call(self.pppoeStart, shell=True)
        if ps != 0:
            ps = subprocess.call(self.pppoeStart, shell=True)
            if ps != 0:
                with open(self.pppoeLog, 'a') as f:
                    f.write(date + '  ' + '拨号程序故障，将杀死所有python进程\n')
                    subprocess.call("ps -aux | grep '/home/qingbo/' | grep -v 'grep' | awk '{print $2}' | xargs kill", shell=True)
            else:
                with open(self.ipLog, 'a') as f:
                    ip = self.getIP()
                    f.write(date + ' 拨号ip：' + ip + '\n')
        else:
            with open(self.ipLog, 'a') as f:
                ip = self.getIP()
                f.write(date + ' 拨号ip：' + ip + '\n')
    ...
    def checkUseful(self):
        ...
        while True:
            if self.port == '33381':
                result = os.popen('curl -s "http://114.215.206.140:8118/syncproxy?machine_id=$HOSTNAME&port=33381&source=zhengwen_curl&purpose=指数微信正文采集"').read()
            else:
...
def main():
    ...
    ipLog = '/mnt/ip.log'
    p = Proxy(pppoeLog, checkUsefullog, ipLog, port)
    p.getPppoe()
